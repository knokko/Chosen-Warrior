...
public class GuiOptionSliceButton extends GuiButton {
	...
	public GuiOptionSliceButton(Vector2f position, Vector2f scale, Color backGround, Color textColor, String text, float minValue, float maxValue, String variable, Class<?> type) {
		super(position, scale, backGround, textColor, text);
		this.minValue = minValue;
		this.maxValue = maxValue;
		deltaValue = maxValue - minValue;
		marker = new GuiTexture(Resources.getFilledTexture(textColor, Material.GUI), new Vector2f(position), new Vector2f(0.01f, scale.y));
		Class<?> options = Game.getOptions().getClass();
		invokeTarget = Game.getOptions();
		try {
		...
		try {
			setter = options.getMethod("set" + variable, type);
		} catch (Exception ex) {
